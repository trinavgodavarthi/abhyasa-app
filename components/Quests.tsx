...
const Quests: React.FC = () => {
  ...
  const handleAddTask = async (e: React.FormEvent) => {
    ...
    await addQuest(newTitle, newDiff);
    setNewTitle('');
    setShowAdd(false);
  };

  // Function to suggest quests using Gemini
  const handleAISuggest = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        Include a difficulty (easy, medium, or hard). Return as JSON.`,
        config: {
  ...
  const activeQuests = [...tasks].filter(t => !t.completed).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  return (
    <div className="bg-rpg-paper border-4 border-rpg-slate shadow-xl p-8 rounded-sm relative min-h-[600px] animate-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="relative z-10">
        <div className="flex justify-between items-center mb-10 border-b-4 border-rpg-slate/20 pb-4">
           <div>
             ...
             <p className="text-[#5c4033] font-bold text-sm tracking-wide uppercase">Active Missions: {activeQuests.length}</p>
           </div>
           <button 
            ...
            <div className="space-y-2">
              {completedQuests.map(task => (
                <div key={task.id} className="flex items-center gap-4 bg-black/5 p-3 rounded group">
                  ...
                  <span className="flex-1 font-bold text-sm text-[#3d2b1f]/70 line-through truncate">{task.title}</span>
                  <button 
      ...
      {showAdd && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-rpg-deep-slate p-1 rounded border-4 border-rpg-slate shadow-2xl max-w-lg w-full">
              ...
              <form onSubmit={handleAddTask} className="space-y-8">
                ...
                <div>
                   ...
                   <div className="grid grid-cols-3 gap-4">
                      <DiffBtn active={newDiff === 'easy'} onClick={() => setNewDiff('easy')} label="EASY" />
                      <DiffBtn active={newDiff === 'medium'} onClick={() => setNewDiff('medium')} label="MEDIUM" />
                      <DiffBtn active={newDiff === 'hard'} onClick={() => setNewDiff('hard')} label="HARD" />
                   </div>
                </div>
...
const QuestItem = ({ task, onComplete, onDelete }: any) => {
      ...
      <div className="flex items-center p-4 gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            <span className={`text-white text-[8px] font-pixel px-2 py-0.5 rounded ${getDiffColor(task.difficulty)} tracking-widest uppercase`}>{task.difficulty}</span>
...
const DiffBtn = ({ active, onClick, label }: any) => (
  <button 
    type="button"
    onClick={onClick}
    className={`p-3 border-2 transition-all ${active ? `bg-primary border-white text-black` : 'bg-black/40 border-rpg-slate text-gray-500'} flex flex-col items-center justify-center font-pixel text-[10px]`}
  >
    {label}
  </button>
);
